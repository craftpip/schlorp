...
- **2026-09-18 — IMPLEMENTED, awaiting user confirm:** `Media.jsx` now builds `stackMenuEntries` (memo over `folderStacks`: first-item basename matched against `items`, thumb via `thrumb()`, count/value carried through) and passes it as `stacks` to `<MediaContextMenu>`. `MediaContextMenu.jsx` renders the add-to-stack section as a single 3-column grid (`repeat(3, 60px)`, `data-testid="media-ctx-stack-grid"`): own/member stacks first, then rest (≤5 preserved); each `StackCell` (`data-testid="media-ctx-add-stack"`) shows the first-item thumbnail (`objectFit: cover`, `bi-layers` icon placeholder when no thumb), a top-right count badge, a top-left `✓` for member stacks, `title` tooltip = stack name, accent border on hover, same disabled/click semantics as before. Menu `minWidth` 180 → 200. `npm run build` passes, `npm run lint` exit 0 (new dep-array warnings match the file's existing pattern).