...
function Group({ title, sub, rows, sections, dimmed }) {
  return (
    <div style={{ opacity: dimmed ? 0.55 : 1, minWidth: 0 }}>
      ...
      {sections ? (
        <div style={{ display: "grid", gap: 10 }}>
          {sections.map((s, i) => (
            <div key={i}>
              <div style={{ fontSize: 10, fontWeight: 700, letterSpacing: ".08em", textTransform: "uppercase", color: "var(--faint)", marginBottom: 2 }}>{s.heading}</div>
              <div style={{ display: "grid", gap: 2 }}>
                {s.rows.map((r, j) => <Row key={j} r={r} />)}
              </div>
            </div>
          ))}
        </div>
    ...
    >
      ...
      >
        ...
        <div className="shcut-cols" style={{ display: "grid", gridTemplateColumns: "1fr 1.4fr", gap: 24, alignItems: "start" }}>
          <Group title="Library" sub="Available when no player is open" sections={LIBRARY_SECTIONS} dimmed={active === "viewer"} />
          <Group title="Player open" sub="Available while viewing a file" sections={VIEWER_SECTIONS} dimmed={active === "library"} />
        </div>
