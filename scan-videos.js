...
async function launchBrowser(chromePath, userDataDir, profileDir) {
...
async function buildBrowserFromLocalProfile() {
  const chromePath = await getChromePath();
  const { userDataDir, profileDir } = resolveProfileConfig();
  return launchBrowser(chromePath, userDataDir, profileDir);
}
...
function extractDownloadableVideoUrls(videoUrls) {
  ...
  const hasVideoLikeCandidate = output.some((u) => !isInstagramAudioOnlyUrl(u));
  const filtered = hasVideoLikeCandidate
    ? output.filter((u) => !isInstagramAudioOnlyUrl(u))
    : output;

  return filtered.sort((a, b) => scoreDownloadCandidate(b) - scoreDownloadCandidate(a));
}
...
(async () => {
  ...
  const isLoginCommand = commandArg === "login-instagram";
  const isDownloadCommand = commandArg === "download-instagram";
  ...
  if (isLoginCommand) {
    ...
    await waitForEnter("Instagram login page is open. Log in now.");
    console.log("Login window closed.");
    await browser.close();
    return;
  }

  const inputUrl = await askUrlIfMissing(isDownloadCommand ? args[1] : commandArg);
  ...
  await waitForEnter(
    "Target page is open. If needed, log in and make sure the reel is visible."
  );
  ...
  if (isDownloadCommand) {
    ...
    if (!downloadableUrls.length) {
      ...
      for (const candidate of downloadableUrls) {
        try {
      ...
      console.log(`\nDownloaded first media to: ${result.filePath}`);
      console.log(`Source URL: ${result.url}`);
    }
  }

  await waitForEnter("Scan complete.");
  await browser.close();
})().catch((err) => {
